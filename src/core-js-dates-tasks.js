...
function getNextFriday(date) {
  ...
  data.setUTCDate(data.getUTCDate() + daysToFriday);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(diff / secondsInDay) + 1;
}
...
function formatDate(date) {
  ...
  const midnightTime = hours >= 12 ? 'PM' : 'AM';
  hours %= 12;
  if (hours === 0) hours = 12;

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${midnightTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    const getCurrDay = new Date(year, month - 1, i).getDay();
    if (getCurrDay === 0 || getCurrDay === 6) weekends += 1;
  }
